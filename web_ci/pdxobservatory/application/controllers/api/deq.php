...
class Deq extends CI_Controller {

  function __construct( ) {
    parent::__construct( );
//    $this->load->model("types");
    $this->load->model("deq_model", "deq" );
  }

  public function deq_stations ( $bounds = null ) {
    $data = array ( );
    $data['start'] = $_GET['start'];
    $data['end'] = $_GET['end'];
    $data['start-time'] = $_GET['start-time'];
    $data['end-time'] = $_GET['end-time'];
    $data['deq_measure'] = $_GET['deq-measure'];
    ...
    $data['geojson'] = $this->deq->get_measured_stations ( $bounds, $data );
  ...
  public function download ( ) {
    ...
    foreach ( $results as $row ) {
      fputcsv ( $fh, array ( $row->station_key, $row->geojson, $data['start'] . ' ' . $data['start-time'], $data['end'] . ' ' . $data['end-time'], $row->$data['deq_measure'], $row->num_measures ) );
    }
  }
}
